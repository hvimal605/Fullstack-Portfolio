...
export const getAllSoftwareApplications = () => async (dispatch) => {
  ...
  try {
    ...
    // console.log('ye softwares h',response);
    
    dispatch(
      softwareApplicationSlice.actions.getAllsoftwareApplicationsSuccess(
        response.data.softwareApplicationsRes
      )
    );
...
export const addNewSoftwareApplication = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      "https://portfolio-backend-code.onrender.com/api/v1/softwareApplication/add",
      data,
      {
        withCredentials: true,
...
export const deleteSoftwareApplication = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `https://portfolio-backend-code.onrender.com/api/v1/softwareApplication/delete/${id}`,
      {
        withCredentials: true,
      }
    );
